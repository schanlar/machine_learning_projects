...
def make_data(
    ...
    assert kind in ["blobs", "moons"], "kind can be either \"blobs\" or \"moons\""
...
def main():
    ...
    plot_unclustered_data(X_std, savefig=False)

    # Initialize KMeans
    number_of_clusters = 2
    km_model = KMeans(n_clusters=number_of_clusters, init="random", random_state=0)

    # Fit the data and predict cluster labels
    km_model.fit(X_std)
    predict_y = km_model.predict(X_std)  
